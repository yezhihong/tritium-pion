...
const double GC_Cut = 1000.;
const double EP_Cut = 0.80;

//int main(){    
//void Yield(){    
void Yield(TString aInputFileName){    
   ...
   // int kin = 0; cout<<"--- Kin (1 or 2 or 3 ...) : "; cin >> kin;
    ...
    TString cut_e_L = Form("DL.evtypebits>>2&1 && L.tr.n==1 && L.cer.asum_c>%4.1f && (L.prl1.e+L.prl2.e)/(L.gold.p*1000.)>%3.2f && (rpl.z)>%4.3f && (rpl.z)<%4.3f && (L.tr.tg_ph)>%4.3f && (L.tr.tg_ph)<%4.3f && (L.tr.tg_th)>%4.3f && (L.tr.tg_th)<%4.3f && (L.tr.tg_dp)>%4.3f && (L.tr.tg_dp)<%4.3f", GC_Cut, EP_Cut, TG_VZ_Min, TG_VZ_Max,TG_Phi_Min, TG_Phi_Max, TG_Theta_Min, TG_Theta_Max, TG_Dp_Min, TG_Dp_Max);
    ...
    t1->DrawLatex(0.2, 0.60, Form("    Normalized Yield = %6.3e #pm %2.1f%% nb/GeV/sr", XS, XS_Err/XS*100.));
